...
function RandomStream( a, b, c, options ) {
	...
	if ( !isNumber( a ) || isnan( a ) ) {
		throw new TypeError( format( '0rx6v,NI', a ) );
	}
	if ( !isNumber( b ) || isnan( b ) ) {
		throw new TypeError( format( '0rx6w,NJ', b ) );
	}
	if ( !isNumber( c ) || isnan( c ) ) {
		throw new TypeError( format( '0rx7C,NR', c ) );
	}
	if ( !(a <= c && c <= b) ) {
		throw new RangeError( format( '0rx9C,HP', 'a <= c <= b', a, b, c ) );
	}
